...
func TestMemoryStorage_SetUser(t *testing.T) {
	...
	s := newTestMemoryStorage()
	s.SetUser(trackerdata.User{ID:"1"})
...
func TestMemoryStorage_SetEntry(t *testing.T) {
	...
	entry := trackerdata.Entry{ID:"b", UserID:"2", EntryType:"1", StartTime:time.Now(), Length: duration}
	s, _ := NewStorage()
	...
	status, _ := s.SetUser(trackerdata.User{ID:"2"})

	if status != trackerdata.StatusCreated {
...
func newTestMemoryStorage() (s* MemoryStorage) {
	...
	s.users["1"] = trackerdata.User{ID: "1"}
	s.entries["1"] = make(map[string]trackerdata.Entry)
	s.entries["1"]["a"] = trackerdata.Entry{ID: "a",
		UserID:    "1",
		EntryType: "1",
		StartTime: time.Now(),
		Length: duration,
